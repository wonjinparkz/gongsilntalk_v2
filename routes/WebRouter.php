...
Route::controller(SharePcController::class)->group(function () {
    ...
    Route::get('/share/calculator/loan/detail/{id}', 'shareCalculatorLoanDetail')->name('share.calculator.loan.detail.view'); // 공유된 수익률 계산기F
});
...
Route::controller(MapPcController::class)->group(function () {
    ...
    Route::get('/map/mobile/product/detail', 'mapMobileProductDetail')->name('www.map.mobile.product.detail');
});
...
Route::middleware('pc.auth')->controller(UserPcController::class)->group(function () {
    Route::get('/mypage/main', 'mypageMainView')->name('www.mypage.product.magagement.main.view');
    // 사용자 매물 관리
