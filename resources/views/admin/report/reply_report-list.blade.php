            ...
            <div class="app-container container-xxl">
                ...
                <div class="card card-flush shadow-sm">
                    <form class="form card-body row border-top p-9 align-items-center" method="GET"
                        action="{{ route('admin.report.reply.list.view') }}">
                        @csrf
                        {{-- 신고한 회원 닉네임 --}}
                            ...
                            <tbody class="fw-semibold text-gray-600">
                                @foreach ($result as $report)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="fw-bold fs-5">
                                                {{ $report->reply->users->type == 0 ? '일반 회원' : '중개사 회원' }}
                                            </span>
                                        </td>
                                        ...
                                        {{-- 게시 상태 --}}
                                        <td class="text-center">
                                            @if ($report->reply->is_delete == 0)
                                                @if (strpos($report->reply->target_type, Community::Class))
                                                    <a href="{{ route('admin.community.detail.view', [$report->reply->target_id]) }}"
                                                        class="text-gray-800 text-hover-primary fs-5 fw-bold">
                                                    @else
                                                        <a href="{{ route('admin.magazine.detail.view', [$report->reply->target_id]) }}"
                                                            class="text-gray-800 text-hover-primary fs-5 fw-bold">
                                                @endif
                                                원글보기
                                                </a>
