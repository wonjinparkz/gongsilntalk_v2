            ...
            <div class="app-container container-xxl">
                ...
                <div class="card card-flush shadow-sm">
                    <form class="form card-body row border-top p-9 align-items-center" method="GET"
                        action="{{ route('admin.popup.list.view') }}">
                        @csrf

                        {{-- 팝업명 --}}
                        <div class="col-lg-6 row mb-6">
                            <label class="col-lg-4 col-form-label fw-semibold fs-6">팝업명</label>
                        ...
                        {{-- 상태 선택 --}}
                        <div class="col-lg-6 row mb-6">
                            <label class="col-lg-4 col-form-label fw-semibold fs-6">공개 여부</label>
                            @php
                        ...
                        {{-- 노출대상 --}}
                        <div class="col-lg-6 row mb-6">
                            <label class="col-lg-4 col-form-label fw-semibold fs-6">노출대상</label>
                            @php
                            ...
                            <div class="col-lg-8 fv-row">
                                <select name="type" class="form-select form-select-solid" data-control="select2"
                                    ...
                                    <option value="0" @if ($type == 0) selected @endif>PC
                                    </option>
                                    <option value="1" @if ($type == 1) selected @endif>모바일
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <form id="orderUpdate" action="{{ route('admin.popup.order.update') }}" method="POST">
                        @csrf
                        ...
                        <table id="notice_table"
                            ...
                            <thead>
                                <tr class="text-start text-gray-400 fw-bold fl-7 text-uppercase gs-0">
                                    ...
                                    <th class="text-center min-w-250px">팝업명</th>
                                ...
                                @foreach ($result as $popup)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="fw-bold fs-5">{{ $popup->id }}</span>
                                        </td>

                                        <td class="text-center">
                                            <input class="fw-bold fs-5 w-100px max-w-300px text-center setorder"
                                                type="text" name="setorder" id="setorder_{{ $popup->id }}"
                                                value="{{ $popup->order }}">
                                            <input class="setid" type="hidden" name="setid"
                                                id="setid_{{ $popup->id }}" value="{{ $popup->id }}">
                                        </td>

                                        {{-- 팝업명 --}}
                                        <td class="text-center">
                                            <a href="{{ route('admin.popup.detail.view', [$popup->id]) }}"
                                                class="text-gray-800 text-hover-primary fs-5 fw-bold">{{ $popup->name }}</a>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <span class="fw-bold fs-5">
                                                ...
                                                {{ $carbon::parse($popup->created_at)->format('Y.m.d') }}
                                            </span>
                                        </td>
                                        {{-- 동작 : 수정, 삭제 --}}
                                        <td class="text-center">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <form action="{{ route('admin.popup.state.update') }}"
                                                        ...
                                                        <input type="hidden" name="is_blind"
                                                            value="{{ $popup->is_blind }}" />
                                                        <a href="#" onclick="parentNode.submit();"
                                                            class="menu-link px-3">
                                                            @if ($popup->is_blind == 0)
                                                                비공개
                                                            @elseif ($popup->is_blind == 1)
                                                                공개
                                                            @endif
                                                        </a>
                                                    </form>
                                                ...
                                                {{-- 삭제 --}}
                                                <div class="menu-item px-3">
                                                    <form id="deleteNotice{{ $popup->id }}"
                                                        action="{{ route('admin.popup.delete') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="id"
                                                            value="{{ $popup->id }}" />
                                                    </form>
                                                    <a href="javascript:deleteAlert({{ $popup->id }});"
