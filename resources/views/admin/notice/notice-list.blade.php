        ...
        <div class="app-toolbar py-3 py-lg-6">
            <div class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="d-inline-block position-relative">
                    <h1 class="page-heading d-flex text-dark fw-bold fs-5ts flex-column justify-content-center ">공지사항
                    </h1>
                    <span
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="{{ route('admin.notice.create.view') }}" class="btn btn-lm fw-bold btn-primary">등록</a>
                </div>
            </div>
        </div>
        {{-- 메인 내용 --}}
            ...
            <div class="app-container container-xxl">
                ...
                <div class="card card-flush shadow-sm">
                    <form class="form card-body row border-top p-9 align-items-center" method="GET"
                        action="{{ route('admin.notice.list.view') }}">
                        @csrf
                                ...
                                @foreach ($result as $notice)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="fw-bold fs-5">{{ $notice->id }}</span>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <a href="{{ route('admin.notice.detail.view', [$notice->id]) }}"
                                                    class="text-gray-800 text-hover-primary fs-5 fw-bold">{{ $notice->title }}</a>
                                                @if ($notice->images != null && count($notice->images) > 0)
                                                    <div class="ms-2 badge badge-light-success">
                                                        이미지 {{ count($notice->images) }} 개
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <form action="{{ route('admin.notice.state.update') }}"
                                                        method="POST">
                                                        @csrf
                                                        <input type="hidden" name="id"
                                                            value="{{ $notice->id }}" />
                                                        <input type="hidden" name="is_blind"
                                                            value="{{ $notice->is_blind }}" />
                                                        <a href="#" onclick="parentNode.submit();"
                                                            class="menu-link px-3">
                                                            @if ($notice->is_blind == 0)
                                                                비공개
                                                            @elseif ($notice->is_blind == 1)
                                                                공개
                                                            @endif
                                                        </a>
                                                    </form>
                                                ...
                                                {{-- 삭제 --}}
                                                <div class="menu-item px-3">
                                                    <form id="deleteNotice{{ $notice->id }}"
                                                        action="{{ route('admin.notice.delete') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="id"
                                                            value="{{ $notice->id }}" />
                                                    </form>
                                                    <a href="javascript:deleteAlert({{ $notice->id }});"
