    ...
    <div class="body">
        ...
        <form method="get" action="{{ route('www.mypage.service.update.second.view') }}" id="update_form"
            ...
            <div class="inner_mid_wrap m_inner_wrap mid_body">
                <h1 class="t_center only_pc">자산 수정하기 <span class="step_number"><span class="txt_point">1</span>/4</span>
                </h1>
                ...
                <div class="offer_step_wrap">
                    ...
                    <div class="box_01 box_reg">
                        ...
                        <input type="hidden" name="address_lng" id="address_lng"
                            value="{{ $result->asset_address->address_lng }}">
                        <input type="hidden" name="address_lat" id="address_lat"
                            value="{{ $result->asset_address->address_lat }}">
                        <input type="hidden" name="region_code" id="region_code"
                            value="{{ $result->asset_address->region_code }}">
                        <input type="hidden" name="region_address" id="region_address"
                            value="{{ $result->asset_address->region_address }}">
                        <input type="text" name="address" id="address"
                            value="{{ $result->asset_address->address }}" style="display: none;">
                        <input type="hidden" name="old_address" id="old_address"
                            value="{{ $result->asset_address->old_address }}">

                        <input type="hidden" name="asset_address_id" id="asset_address_id"
                            value="{{ $result->asset_address->id }}">

                        <div class="address_reg_wrap">
                            <div class="inner_item">
                                <div
                                    class="search_address_1  {{ $result->asset_address->is_temporary == 0 ? 'active' : '' }}">
                                    <button class="btn_graylight_ghost btn_full_thin txt_r" type="button"
                                        onclick="getAddress()">주소
                                        검색</button>
                                </div>
                                <div
                                    class="search_address_2 {{ $result->asset_address->is_temporary == 0 ? '' : 'active' }}">
                                    <button class="btn_graylight_ghost btn_full_thin txt_r" type="button"
                                ...
                                <div class="inner_address">
                                    <div class="address_row" id="roadName">
                                        ...
                                        {{ $result->asset_address->address }}
                                    </div>
                                    <div class="address_row" id="jibunName">
                                        @if ($result->asset_address->old_address != '')
                                            <span>
                                                지번
                                            </span>
                                            {{ $result->asset_address->old_address }}
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="detail_address_2 mt18 active">
                                    <div>
                                        <input type="text" id="address_detail" name="address_detail"
                                            ...
                                            {{ $result->address_detail == '' && $result->asset_address->is_temporary == 1 ? 'disabled' : '' }}
                                            placeholder="건물명, 동/호 또는 상세주소 입력 예) 1동 101호">
                                    </div>
                                    <div class="mt8">
                                        <input type="checkbox" name="address_no" id="address_no_2" value="Y"
                                            {{ $result->address_detail == '' && $result->asset_address->is_temporary == 1 ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        $("#type_detail_{{ $result->type_detail + 1 }}").trigger('click');
