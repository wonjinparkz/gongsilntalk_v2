...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('corp_product_price', function (Blueprint $table) {
            ...
            $table->bigInteger('price')->comment('매매 - 매매가, 전세 - 전세가, 전매 - 전매가, 월세&단기임대&임대 - 보증금');
            $table->bigInteger('month_price')->nullable()->comment('월 임대료');
            $table->bigInteger('premium_price')->nullable()->comment('상가 - 권리금, 분양권 - 프리미엄 금액');
            $table->double('acquisition_tax', 10,2)->nullable()->comment('취득세율 소수점 2자리까지');
            $table->bigInteger('support_price')->nullable()->comment('지원금액');
            ...
            $table->bigInteger('invest_price')->nullable()->comment('투자 보증금');
            $table->bigInteger('invest_month_price')->nullable()->comment('투자 월임대료');
            $table->timestamps();
        });
