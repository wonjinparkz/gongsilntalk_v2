...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('data_store', function (Blueprint $table) {
            ...
            $table->string('kstoreName')->nullable()->comment('상가 이름');

            // 상가 기본 정보
            $table->string('kstoreAddr')->nullable()->comment('법정동주소');
            $table->string('codeSaleNm')->nullable()->comment('분양형태');
            $table->string('codeHeatNm')->nullable()->comment('난방방식');
            $table->string('kstoreTarea')->nullable()->comment('건축물대장상 연면적(㎡)');
            $table->string('kstoreDongCnt')->nullable()->comment('동수');
            $table->string('kstoredaCnt')->nullable()->comment('세대수');
            $table->string('kstoreBcompany')->nullable()->comment('시공사');
            $table->string('kstoreAcompany')->nullable()->comment('시행사');
            $table->string('kstoreTel')->nullable()->comment('관리사무소연락처');
            $table->string('kstoreFax')->nullable()->comment('관리사무소팩스');
            $table->string('kstoreUrl')->nullable()->comment('홈페이지주소');
            ...
            $table->string('kstoreUsedate')->nullable()->comment('사용승인일');
            $table->string('kstoreMarea')->nullable()->comment('관리비부과면적(㎡)');
            $table->string('kstoreMparea_60')->nullable()->comment('60㎡ 이하');
            $table->string('kstoreMparea_85')->nullable()->comment('60㎡ ~ 85㎡ 이하');
            $table->string('kstoreMparea_135')->nullable()->comment('85㎡ ~ 135㎡ 이하');
            $table->string('kstoreMparea_136')->nullable()->comment('135㎡ 초과');
            $table->string('privArea')->nullable()->comment('단지 전용면적합(㎡)');


            // 상가 상세 정보
            $table->string('codeMgr')->nullable()->comment('일반관리방식');
            $table->string('kstoreMgrCnt')->nullable()->comment('일반관리인원');
            $table->string('kstoreCcompany')->nullable()->comment('일반관리 계약업체(자치관리일 경우 없을 수 있음)');
            $table->string('codeSec')->nullable()->comment('경비관리방식');
            $table->string('kstoredScnt')->nullable()->comment('경비관리인원');
            $table->string('kstoredSecCom')->nullable()->comment('경비관리 계약업체(자치관리일 경우 없을 수 있음)');
            $table->string('codeClean')->nullable()->comment('청소관리방식');
            $table->string('kstoredClcnt')->nullable()->comment('청소관리인원');
            $table->string('codeGarbage')->nullable()->comment('음식물처리방법');
            $table->string('codeDisinf')->nullable()->comment('소독관리방식');
            $table->string('kstoredDcnt')->nullable()->comment('소독관리 연간 소독횟수');
            $table->string('disposalType')->nullable()->comment('소독방법');
            $table->string('codeStr')->nullable()->comment('건물구조');
            $table->string('kstoredEcapa')->nullable()->comment('수전용량');
            ...
            $table->string('kstoredEcnt')->nullable()->comment('승강기대수(승객용+화물용+승객/화물+장애우+비상용+기타)');
            $table->string('kstoredPcnt')->nullable()->comment('주차대수(지상)');
            $table->string('kstoredPcntu')->nullable()->comment('주차대수(지하)');
            $table->string('codeNet')->nullable()->comment('주차관제.홈네트워크');
            $table->string('kstoredCccnt')->nullable()->comment('CCTV 카메라 수');
            $table->string('welfareFacility')->nullable()->comment('부대시설 및 복리시설');
            $table->string('kstoredWtimebus')->nullable()->comment('버스정류장 거리');
            $table->string('subwayLine')->nullable()->comment('지하철호선');
            $table->string('subwayStation')->nullable()->comment('지하철역명');
            $table->string('kstoredWtimesub')->nullable()->comment('지하철역 거리');
        ...
        DB::statement("ALTER TABLE data_store COMMENT='상가 정보'");
    }
