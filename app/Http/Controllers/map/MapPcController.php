...
class MapPcController extends Controller
{
    ...
    public function getMapMarker(Request $request)
    {
        ...
        if ($request->mapType == 0) {
            ...
            $agent = User::select('users.id', 'company_address_lng',DB::raw('company_address_lat - 0.00025 as company_address_lat'))->with('image')->where('type', 1)->where('state', 0)->where('company_state', 1)
                ->whereRaw(
